...
3. **The elided test navigation in Task 6.** Two of those three tests need keystrokes through a chooser the plan does not spell out, because the helper signatures live in `launcher.test.tsx`. Read `toConfig`/`toRow` and the neighbouring cases first; do not invent a helper.
