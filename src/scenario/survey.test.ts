...
describe("solveBounds", () => {
	...
	it("stays near the radius it was asked for", () => {
		...
		const { bounds, adjustment } = solveBounds(SEED, { x: 0, y: 0 }, 4 * CHUNK, "cliffs");
	...
	it("keeps the requested style", () => {
		expect(solveBounds(SEED, { x: 0, y: 0 }, 256, "ocean").bounds.style).toBe("ocean");
	});
});
...
describe("styleForEdge", () => {
	it("picks a style the ground can carry", () => {
		...
		// the sea being there.
		for (const seed of SEEDS) {
			const style = styleForEdge(seed, { x: 0, y: 0 }, 256);
...
describe("sitesWithin", () => {
	it("is deterministic and ordered independently of who asked", () => {
		const bounds = solveBounds(SEED, { x: 0, y: 0 }, 256, "cliffs").bounds;
		const once = sitesWithin(SEED, bounds).map((site) => site.id);
		const twice = sitesWithin(SEED, bounds).map((site) => site.id);
		expect(twice).toEqual(once);
	});

	it("finds more when asked to look wider", () => {
		const bounds = solveBounds(SEED, { x: 0, y: 0 }, 256, "cliffs").bounds;
		expect(sitesWithin(SEED, bounds, CHUNK).length).toBeGreaterThanOrEqual(
			sitesWithin(SEED, bounds).length,
		);
	});
});
...
describe("surveyWorld", () => {
	...
	it("reports only sites inside the playable area", () => {
		const survey = surveyWorld(SEED, "medium");
		for (const entry of survey.sites) {
	...
	it("gives a longer scenario a bigger world and more to do", () => {
		const short = surveyWorld(SEED, "short");
		const long = surveyWorld(SEED, "long");
	...
	it("is deterministic", () => {
		expect(surveyWorld(SEED, "short").bounds).toEqual(surveyWorld(SEED, "short").bounds);
	});
