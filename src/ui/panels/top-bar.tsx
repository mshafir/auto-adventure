...
export function TopBar({ state, width, summary, placeName, weather, light }: TopBarProps) {
	...
	const time = clock(state.time);
	const day = `day ${state.time.day}`;
	...
	const optional: { readonly text: string; readonly color: string }[] = [
		...
		...(summary ? [{ text: ground(summary), color: "green" }] : []),
	];
...
function ground(summary: TerrainSummary): string {
	return Object.entries(summary.biomeCounts)
		.sort((a, b) => b[1] - a[1])
		.slice(0, 2)
		.map(
			([biome, count]) =>
				`${biomeDef(biome as never).name} ${Math.round((count / CHUNK_AREA) * 100)}%`,
		)
		.join(" · ");
}
