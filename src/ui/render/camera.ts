...
/**
 * Slide the camera toward a target a fixed number of tiles per call.
 *
 * What a scene's pan is made of. A cut is `cameraCenteredOn`; a pan is this, called once per
 * frame with the camera it produced last time, until it arrives.
 *
 * Deliberately a *rate* rather than a duration. A duration would need the pan to know how
 * many frames it has, which means the scene's pacing would depend on the frame interval —
 * and that belongs to the UI, not to the scene. At a fixed rate a slow terminal shows a
 * slower pan of the same distance rather than a faster one that skips.
 *
 * Not idempotent, unlike {@link cameraFollowing}, and it cannot be: its whole job is to be
 * somewhere new each time it is asked. So it must be called once per frame from an effect or
 * a memo keyed on the frame — never from a render that React may run twice.
 */
export function cameraTowards(
	previous: Camera | undefined,
	...
	const goal = cameraCenteredOn(target, width, height);
	if (!previous || previous.width !== width || previous.height !== height) return goal;
	if (tilesPerFrame <= 0) return goal;
	return {
		x: step(previous.x, goal.x, tilesPerFrame),
		y: step(previous.y, goal.y, tilesPerFrame),
		width,
		height,
	};
}

/** One axis of a pan, stopping exactly on the target rather than oscillating past it. */
